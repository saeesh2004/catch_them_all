...
class LedStatusNode(Node):
    def __init__(self):
        ...
        self.led_states_publisher_=self.create_publisher(LedStatusArray, "led_states", 10)
        self.timer = self.create_timer(4, self.publish_led_states)
        self.set_led_service_=self.create_service(SetLed, "set_led", self.callback_set_led)
        self.get_logger().info("this crappy node has started")
